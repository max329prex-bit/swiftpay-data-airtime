...
// Always return 200 to prevent Korapay retries — but only CREDIT on valid sig
...
serve(async (req) => {
  try {
    ...
    // ── HMAC verification — MANDATORY, no signature = silent drop ────────
    const kSig = req.headers.get("x-korapay-signature") ?? "";
    if (!kSig) {
      // No signature: return 200 to avoid Korapay treating as error,
      // but do NOT process anything.
      console.warn("Korapay webhook: missing signature — request dropped");
      return new Response(OK, { status: 200, headers: OK_HDR });
    }
    const expected = await hmacSha256Hex(KP_SK, rawBody);
    if (kSig !== expected) {
      // Bad signature: same — return 200 but don't process
      console.error("Korapay webhook: invalid HMAC signature — request dropped");
      return new Response(OK, { status: 200, headers: OK_HDR });
    }
    // ─────────────────────────────────────────────────────────────────────
    ...
    if (!ref || !userId || amount < 100) {
      console.warn("Korapay webhook: missing required fields", { ref, userId, amount });
      return new Response(OK, { status: 200, headers: OK_HDR });
    }

    const sb = createClient(SUPA_URL, SUPA_SVC);

    // Idempotency — deduplicate replayed events
    ...
    if (dupErr) {
      console.warn("Korapay webhook: duplicate event", ref);
      return new Response(OK, { status: 200, headers: OK_HDR });
    }
