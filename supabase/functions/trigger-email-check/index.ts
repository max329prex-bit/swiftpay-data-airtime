...
serve(async (req) => {
  ...
  try {
    ...
    // Trigger the IMAP scanner. Pass the deposit_id so the scanner can do a
    // targeted, deeper search across INBOX and All Mail instead of the shallow cron.
    const scanBody = deposit_id ? { deposit_id } : {};
