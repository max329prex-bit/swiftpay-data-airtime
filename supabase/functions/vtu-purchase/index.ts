...
// GSubz airtime mapping: network → GSubz serviceID
...
function isBundleDown(msg: string) {
  ...
  return l.includes("not available") || l.includes("unavailable") || l.includes("out of stock") ||
    ...
    l.includes("invalid plan") || l.includes("plan not found");
}
...
async function bsplugBuy(netId: number, planId: number, phone: string): Promise<PR> {
  try {
    ...
    return { success: false, msg: `BSPlug unreachable: ${e}` };
  }
}
...
async function gsubzBuyRaw(params: Record<string, string>): Promise<PR> {
  ...
  console.log("[gsubz] key length:", key.length);
  if (!key) {
  ...
  try {
    ...
    const d = await r.json();
    console.log("[gsubz] status:", r.status, "response:", JSON.stringify(d).slice(0, 500));
...
// FIX per GSubz docs (https://documenter.getpostman.com/view/16767782/UyxhnnZz):
// - Data: serviceID uses underscores (mtn_sme), amount is EMPTY, plan is numeric
// - Airtime: serviceID is just network (mtn), NO plan field, amount is actual value
async function gsubzData(pkgCode: string, phone: string, reqId: string): Promise<PR> {
  const cleanCode = pkgCode.replace(/^GSZ-/, "");
  const parts = cleanCode.split("-");

  // Extract plan ID (last part)
  const planId = parts[parts.length - 1];

  // Extract serviceID: everything before the last dash, convert dashes to underscores
  // e.g. "mtn-sme" → "mtn_sme", "mtn_cg_lite" stays "mtn_cg_lite"
  const service = parts.length > 1
    ? parts.slice(0, parts.length - 1).join("-").replace(/-/g, "_")
    : "";

  if (!planId || !service) {
    return { success: false, msg: "Gsubz: invalid data plan code" };
  }
  ...
  const params: Record<string, string> = {
    ...
    phone: phone,
    requestID: reqId,
    amount: ""  // FIX: GSubz data requires empty amount field per docs
  };

  console.log("[gsubz-data] serviceID=", service, "plan=", planId);
  return gsubzBuyRaw(params);
}

// ── Provider: GSubz (airtime) ─────────────────────────────
// FIX per GSubz docs:
// - Airtime: serviceID=mtn, api=..., amount=100, phone=...
// - NO plan field, NO requestID for airtime
async function gsubzAirtime(network: string, phone: string, amount: number, reqId: string): Promise<PR> {
  const serviceID = GSUBZ_AIRTIME_MAP[network?.toUpperCase()] || "mtn";

  // GSubz airtime docs: serviceID, api, amount, phone ONLY
  const params: Record<string, string> = {
    serviceID,
    api: GSUBZ_KEY,
  ...
  console.log("[gsubz-airtime] serviceID=", serviceID, "amount=", amount);
  return gsubzBuyRaw(params);
}
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[vtu] request: type=${type} network=${network} phone=${phone} amount=${sellPrice} pkgCode=${pkgCode} prvCode=${prvCode}`);
    ...
    // ── STEP 1: Debit wallet + create PENDING transaction ──
    ...
    if (type === "data" && prvCode === "gsubz") {
      ...
      if (gsubzHealthy) {
        // FIX: Use gsubzData which correctly formats per GSubz docs
      ...
      // FIX: Use gsubzAirtime which correctly formats per GSubz docs
      const reqId = `GSZ-${Date.now()}-${Math.random().toString(36).substr(2, 5).toUpperCase()}`;
      txMeta.gsubz_request_id = reqId;

      pr = await gsubzAirtime(network || "", phone || "", sellPrice, reqId);
      ...
      if (type === "electricity") {
        ...
        extra = { meter: meter_number || phone || "", amount: String(sellPrice || 0) };
      } else {
        serviceID = prvCode || "";
        plan = pkgCode || "";
      }
