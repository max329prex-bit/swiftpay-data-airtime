...
// ── DIAGNOSTIC: Log full validation response to Telegram ────────────────────────────────
async function diagLog(label: string, phone: string, provider: string, status: string, body: unknown) {
  const truncated = JSON.stringify(body).slice(0, 800);
  await tg(`🔍 *DIAG: ${label}*\nProvider: ${provider}\nPhone: ${phone}\nStatus: ${status}\n\n${truncated}`);
}
