...
// Format an amount with naira/kobo display (e.g. ₦150.00).
...
function getErrorMessage(e: any): string {
  ...
  if (e.error) {
    ...
    if (typeof e.error === "object" && e.error.message && typeof e.error.message === "string") {
...
export default function FreeTransferPanel() {
  ...
  const loadDefaultsAndDeposit = useCallback(async () => {
    ...
    if (dep) {
      ...
      // current user. The DB query is user-scoped, so this check is a safety net.
    ...
    // No reserved session in DB for this user: show the normal setup/amount flow.
    setDeposit(null);
  ...
  useEffect(() => {
    ...
    // Always clear the pay screen when the user signs out so the next account on
    // this browser never sees the previous one's deposit details.
  ...
  // Always clear the pay screen if it belongs to a different user. This is the
  // final safety net for the same-browser multi-account leak.
  useEffect(() => {
    if (deposit && user?.id && deposit.user_id !== user.id) {
      setDeposit(null);
      setStep("amount");
    }
  }, [deposit, user?.id]);
  ...
  async function handleCreate() {
    ...
    try {
      ...
      if (!ok || data.error) throw new Error(getErrorMessage(data.error) || "Could not create deposit");
      ...
      toast.error(getErrorMessage(e) || "Failed to create deposit");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function handleTriggerCheck() {
    ...
    try {
      ...
      const trigger = async () => {
        ...
        if (!ok || data.error) throw new Error(getErrorMessage(data.error) || "Could not trigger check");
        return data;
      };
      ...
      toast.error(getErrorMessage(e) || "Failed to check payment");
      setStep("pay");
    }
  }
        ...
        {step === "pay" && deposit && (
          <motion.div key="pay" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
            ...
            <div className="rounded-2xl bg-emerald-500/5 border border-emerald-500/20 p-4 flex gap-3">
              ...
              <p className="text-sm text-emerald-100/80">
                Send the <strong>EXACT amount</strong> (e.g. {nairaExact(deposit.amount)}) from your <strong>{override ? overrideBankName : profile.ft_bank_name}</strong> bank.
              </p>
            </div>
