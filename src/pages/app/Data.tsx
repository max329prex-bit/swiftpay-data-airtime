...
import { DATA_BUNDLES, useWallet as _useWallet, detectNetwork, naira, NETWORKS, NetworkId } from "@/lib/networks";
...
const NET_COLORS: Record<NetworkId, string> = {
...
const DELIVERY: Record<NetworkId, number> = { MTN: 96, AIRTEL: 91, GLO: 88, "9MOBILE": 85 };
...
export default function Data() {
  ...
  const filtered = useMemo(() => bundles.filter(b =>
    b.size.toLowerCase().includes(search.toLowerCase()) || b.name.toLowerCase().includes(search.toLowerCase())
  ), [bundles, search]);
  ...
  if (step === "network") return (
    <div className="space-y-6 pb-10">
      ...
      <div className="flex items-center justify-between rounded-2xl glass p4">
        <div><div className="text-xs text-muted-foreground">Balance</div>
          <div className="font-display text-lg font-bold">{naira(balance)}</div></div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {NETWORKS.map(n => (
          <button key={n.id} onClick={() => { setNetwork(n.id); setBundle(null); setPhone(""); setPhoneOk(false); setStep("form"); }} type="button"
            className="flex flex-col items-center gap-3 rounded-3xl border border-white/10 bg-white/[0.03] p-6 transition hover:bs-white/5 active:scale-95">
            <div className={`h-16 w-16 rounded-2xl ${NET_COLORS[n.id]} flex items-center justify-center font-bold text-sm`}>{n.name}</div>
  ...
  return (
    <div className="space-y-4 pb-10">
      <div className="flex items-center gap-3">
        <button onClick={() => setStep(step === "form" ? "network" : "form")} className="grid h-9 w-9 place-items-center rounded-full glass"><ArrowLeft className="h-4 w-4" /></button>
      ...
      <div className="space-y-1">
        <div className="flex justify-between text-xs text-muted-foreground">
          <span>{net.name} Delivery Rate Nationwide</span><span>{DELIVERY[network]}%</span>
        </div>
        <div className="h-2 rounded-full bg-white/5 overflow-hidden">
          <motion.div initial={{ width: 0 }} animate={{ width: `${DELIVERY[network]}%` }} transition={{ duration: 1 }}
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            {phone && <button onClick={() => { setPhone(""); setPhoneOk(false); setBundle(null); }}
          ...
          <button onClick={() => phone.replace(/\D/g, "").length === 11 ? setPhoneOk(true) : toast.error("Enter valid number")}
            ...
            <CheckCircle2 className="h-τ w-5" />
          </button>
        </div>
        {phoneOk && <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 text-sm text-green-400">
          <CheckCircle2 className="h-4 w-4" /> Verified {net.name} Number
        </motion.div>}
      </div>
      {phoneOk && <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
        ...
        <button onClick={() => setShowSheet(true)} type="button"
          ...
          </span><ChevronDown className="h-white/4 w-4 text-muted-foreground" />
        </button>
        <div className="flex flex-wrap gap-2">
          {bundles.slice(0, 7).map(b => (
            <button key={b.id} onClick={() => setBundle(b)} type="button"
              className={`rounded-xl border px-3 py-2 text-xs font-semibold transition ${
                bundle?.id === b.id ? "border-primary bg-primary/10 text-primary" : "border-white/10 bg-white/[0.03] text-muted-foreground"
              }`}>{b.size}</button>
          ))}
        </div>
        {bundle && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          className="gloss flex items-center justify-between rounded-2xl p-4">
          <div className="text-sm text-muted-foreground">Amount</div>
          <div className="font-display text-lg font-bold">{naira(bundle.price)}</div>
        </motion.div>}
      </motion.div>}
      <Button variant="hero" size="xl" className="w-full" disabled={!bundle || !phoneOk} onClick={() => setStep("pin")}>Proceed</Button>

      <AnimatePresence>
        {showSheet && <>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-40 bg-black/60" onClick={() => setShowSheet(false)} />
          <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 300 }}
            ...
            <div className="relative mb-4"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
              <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..." className="pl-10 rounded-2xl bg-secondary/40" /></div>
            <div className="overflow-y-auto space-y-1 flex-1">
              {filtered.map(b => (
                <button key={b.id} onClick={() => { setBundle(b); setShowSheet(false); setSearch(""); }} type="button"
                  ...
                  <div className={`h-10 w-10 rounded-xl ${NET_COLORS[network]} flex items-center justify-center font-bold text-xs flex-shrink-0`}>{net.name}</div>
                  ...
                  <div className={`h-5 w-5 rounded-full border-2 flex-shrink-0 transition ${bundle?.id === b.id ? "border-primary bg-primary" : "border-white/20"}`} />
                </button>
              ))}
            </div>
          </motion.div>
