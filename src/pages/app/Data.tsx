...
export default function Data() {
  ...
  const tabPlans = netPlans.filter(p => p.duration === duration && (!hideGiftPlans || !isNonOwingPlan(p)));
            ...
            {network === "MTN" ? (
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  {tabPlans.filter(p => !p.requires_non_owing_line).length === 0 ? (
                    ...
                    <>
                      <div className="grid grid-cols-3 gap-2">
                        {tabPlans.filter(p => !p.requires_non_owing_line).slice(0, showMore ? tabPlans.filter(p => !p.requires_non_owing_line).length : Math.min(6, tabPlans.filter(p => !p.requires_non_owing_line).length)).map(p => (
                ...
                <div className="space-y-2">
                  ...
                  {tabPlans.filter(p => p.requires_non_owing_line).length === 0 ? (
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {tabPlans.filter(p => p.requires_non_owing_line).map(p => (
                        <PlanCard key={p.id} plan={p} selected={plan?.id === p.id} onSelect={pp => setPlan(pp)} />
                      ))}
                    </div>
                  )}
