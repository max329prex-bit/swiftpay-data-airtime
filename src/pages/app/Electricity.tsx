...
export default function Electricity() {
  ...
  async function pay() {
    if (pin.length < 4) return toast.error("Enter 4-digit PIN"); if (amount < 1000) return toast.error("Min ₦1,000"); if (amount > balance) return toast.error("Insufficient balance");
    setBusy(true);
    try {
      const { data, error } = await supabase.functions.invoke("vtu-purchase", {
        body: { type: "electricity", network: provider.aidapay_code, phone: meter, amount, pin, meterType, customerName },
      });
      if (error) throw error;
      if (!data?.success) throw new Error(data?.error || "Purchase failed");
      refresh();
      nav(`/app/success?ref=${data.reference}&type=electricity&amount=${amount}&network=${provider.id}`);
