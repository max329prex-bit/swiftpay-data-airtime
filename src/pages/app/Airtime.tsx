...
// FIX: Removed 50 — GSubz airtime minimum is 100
const QUICK = [100, 200, 500, 1000, 2000];
const MIN_AMOUNT = 100; // GSubz airtime minimum
...
export default function Airtime() {
  ...
  return (
    <div className="space-y-4 pb-10">
      ...
      <div className="space-y-2"><div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">Amount</div><Input value={amount || ""} onChange={e => setAmount(Math.max(0, Number(e.target.value.replace(/\D/g, ""))))} placeholder={`Min ${naira(MIN_AMOUNT)}`} inputMode="numeric" className="h-14 rounded-2xl bg-secondary/40 text-lg font-semibold" /><div className="grid grid-cols-3 gap-2">{QUICK.map(v => (<button key={v} onClick={() => setAmount(v)} type="button" className={`rounded-xl border p-3 text-sm font-semibold transition ${amount === v ? "border-primary bg-primary/10" : "border-white/10 bg-white/[0.03]"}`}>{naira(v)}</button>))}</div></div>
      <Button variant="hero" size="xl" className="w-full" disabled={!phoneOk || amount < MIN_AMOUNT} onClick={() => setStep("pin")}>Proceed</Button>
